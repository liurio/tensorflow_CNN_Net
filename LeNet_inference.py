...
def inference(input_tensor,train,regularizer):
	...
	with tf.variable_scope("layer6-fc2"):
		fc2_weights = tf.get_variable("weight",[nodes,FC_SIZE],initializer=tf.truncated_normal_initializer(stddev=0.1))
		if regularizer!=None:
			tf.add_to_collection('losses',regularizer(fc2_weights))
		fc2_biases = tf.get_variable("bias",[FC_SIZE],initializer=tf.constant_initializer(0.0))
		logit = tf.matmul(reshaped,fc2_weights)+fc2_biases
